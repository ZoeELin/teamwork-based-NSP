...
def parse_observation_file(filename):
    data = []
    comc_weight = None
    week = None
    current_entry = {}
...
def plot_grouped_bar_chart(df, output_filename="extracted/grouped_bar_chart.png"):
...
def plot_score_trend_line(df, output_filename="extracted/score_trend.png"):
    categories = ["Soft", "ComC", "Total"]
...
def plot_comc_per_weight(df, output_filename="extracted/comc_per_weight.png"):
    """
...
def plot_avg_score_by_weight(df, output_filename="extracted/avg_score_trend.png"):
    categories = ["Soft", "ComC", "Total"]
    colors = {"Soft": "tab:blue", "ComC": "tab:orange", "Total": "tab:green"}

    # 擷取 Weight 資訊 (e.g., W50, W100...) 作為 group key
    df["Weight"] = df["Label"].str.extract(r"(W\d+)$")

    # 將數值格式轉成 int，排序方便
    df["WeightInt"] = df["Weight"].str.extract(r"W(\d+)").astype(int)
    ...
    ax.set_title("Average Score by ComC Weight (Initial vs Best)")
    ax.legend()
    ax.grid(True)
    ax.set_xticks(grouped.index)
    ax.set_xlim(grouped.index.min() - 10, grouped.index.max() + 10)
...
def plot_comc_per_weight_by_weight(df, output_filename="extracted/comc_per_weight_by_weight.png"):
    """
...
def plot_total_minus_comc_penalty_by_weight(
    df, output_filename="extracted/total_minus_comc_by_weight.png"
):
    """
...
def main():
    filename = 'procession_with_n040w4_weight0_1000.txt'

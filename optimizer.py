    ...
    for temp_idx in range(total_temp_levels):
        ...
        for inner_idx in range(ns_per_temp):
            ...
            if exceed_nurses != []:
                ...
                mcts_scheduler = NurseSchedulerMCTS(scenario, week_data, nurseHistory, nurses_lastday_from_lastweek)
                
                for nurse_id in exceed_nurses:
                    print(f"\nRescheduling nurse {nurse_id} with MCTS...")
                    nurse_schedule = mcts_scheduler.mcts_search(nurse_id, max_iterations=10)

...
# 重新建構每一輪的 heatmap 矩陣
for i in range(1, n_files + 1):
    filepath = os.path.join(folder, f"coop-intensity-ComC-{i}.json")
    with open(filepath, "r") as f:
        data = json.load(f)
...
fig.suptitle("Cooperation Strength Based on SA-ComC Algorithm", fontsize=20)
...
plt.savefig(os.path.join(folder, "ComC-based_coop_evolution_run12times.png"))
